...
const PREFERRED_DATA_DIR = process.env.DATA_DIR || path.join(ROOT_DIR, "data");
let activeDataDir = PREFERRED_DATA_DIR;
let activeContactsFile = path.join(activeDataDir, "contacts.json");
...
async function ensureStorage() {
  const candidates = [
    ...
    path.join(os.tmpdir(), "studentutilitytools-data")
  ].filter(Boolean);
  ...
  for (const dir of candidates) {
    ...
    try {
      ...
      try {
        await fs.promises.access(contactsPath);
      } catch (missingError) {
        await fs.promises.writeFile(contactsPath, "[]", "utf8");
      }

      activeDataDir = dir;
      activeContactsFile = contactsPath;
      return;
    } catch (error) {
      lastError = error;
    }
  ...
  throw createError(
    500,
    "Unable to initialize writable storage directory. Last error: " +
...
function getStorageInfo() {
  return {
    preferredDataDir: PREFERRED_DATA_DIR,
    activeDataDir
  }
}
...
async function handleContact(payload) {
  ...
  const contacts = JSON.parse(await fs.promises.readFile(activeContactsFile, "utf8"));
  contacts.push(submission);
  await fs.promises.writeFile(activeContactsFile, JSON.stringify(contacts, null, 2), "utf8");
...
function createMailTransporter() {
  ...
  try {
    ...
    throw createError(500, "Mail transport dependency is missing on the server.");
  }
